...
document.addEventListener('keyup', function(e){
	var tgt = e.target;
	if (tgt.classList.contains('country')){
		...
		var matches = [];
		var val = country.value.toLowerCase();

		for (var i = 0; i < countries.length; i++){
			var lowCountry = countries[i].toLowerCase();
			if (lowCountry.indexOf(val) == 0){
		...
		for (var j = 0; j < matches.length; j++){
			var drItem = document.createElement('div');
			drItem.classList.add('dropdown__item');
			drItem.innerHTML = matches[j];
			dropdown.appendChild(drItem);

		}
		dropdown.addEventListener('click', function(e){
			var tgt = e.target;

			if (tgt.classList.contains('dropdown__item')){
				country.value = tgt.innerHTML;
				dropdown.remove();
			}
		})
	}
